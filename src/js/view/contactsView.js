...
class ContactsView extends View {
  ...
  getMarkup() {
    this._markup = Object.entries(this._data);
    // console.log(this._markup[0][1]);
    // console.log(this._markup[1][1]);
    console.log(this._markup[2][1]);
    const markup = this._markup.map((ent) => {
      `
      <div class="user-img__container">
        <img src="${ent[0][1].profileImage}" alt="Profile Image" class="user-profile-img" />
        <input
          type="file"
      ...
      <div class="name-container">
        <label class="contact-name">${ent[0][1].firstName} ${ent[0][1].lastName}</label>
      </div>
    `;
    });
